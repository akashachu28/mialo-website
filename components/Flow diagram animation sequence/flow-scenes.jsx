...
const MOTION = {
  enter: (p, dx = -26) => ({
    ...
    transform: `translate(${(1 - ec(p)) * dx}px, 0px) scale(${lerp(0.92, 1, ec(p))})`,
  }),
  draw: (p) => ({ strokeDasharray: '1 1', strokeDashoffset: 1 - ec(p), pathLength: 1 }),
  pop: (p) => ({ opacity: clamp(p * 1.4, 0, 1), transform: `scale(${lerp(0.8, 1, ec(p))})` }),
};
...
function SourceNode({ s, p, accent }) {
  const st = MOTION.enter(p);
  return (
    <div style={{ position: 'absolute', left: 96, top: s.y - 38, display: 'flex', alignItems: 'center', gap: 26, ...st }}>
      <div style={{
...
function Pill({ label, x, y, p, accent, filled, glow }) {
  const st = MOTION.enter(p, -18);
  return (
    <div style={{
      position: 'absolute', left: x - PW / 2, top: y - PH / 2, width: PW, height: PH,
      borderRadius: 14, display: 'flex', alignItems: 'center', gap: 12, padding: '0 16px',
      ...
      font: `400 21px ${FONT}`, lineHeight: 1.15, ...st,
    }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `radial-gradient(circle at 50% 45%, rgba(70,140,255,${0.30 + 0.10 * pulse}), rgba(8,20,55,0.6))`,
        border: '1px solid rgba(120,180,255,0.4)',
      }} />
      <div style={{
        position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', textAlign: 'center',
        color: '#fff', font: `500 40px/1.15 ${FONT}`, letterSpacing: '-0.01em',
        textShadow: '0 0 26px rgba(60,130,255,0.9)',
...
function BrainHub({ p, prog, accent }) {
  ...
  return (
    <div style={{
      position: 'absolute', left: BRAIN.x - 92, top: BRAIN.y - 92, width: 184, height: 184,
      ...MOTION.pop(p),
    }}>
      <div style={{
        position: 'absolute', inset: 0, borderRadius: '50%',
        border: '1px dashed rgba(90,150,255,0.4)', transform: `rotate(${360 * prog}deg)`,
      }} />
      <div style={{
        ...
        boxShadow: `0 0 ${30 + 26 * pulse}px rgba(45,120,255,0.55)`,
  ...
  return (
    <g>
      <path d={d} stroke="rgba(90,150,255,0.32)" strokeWidth={width} fill="none"
        pathLength="1" strokeDasharray="1 1" strokeDashoffset={1 - ec(p)} />
      <path d={d} stroke={accent} strokeWidth={width + 0.6} fill="none" strokeLinecap="round"
        ...
        opacity={0.9 * ec(p)} style={{ filter: 'drop-shadow(0 0 6px rgba(70,140,255,0.9))' }} />
    </g>
  );
}

function Diagram({ r, prog, accent, glow }) {
  const stag = (v, i, n) => clamp((v - i * (0.5 / n)) / 0.5, 0, 1);
  return (
    <div style={{ position: 'absolute', inset: 0, filter: `saturate(${glow})` }}>
      <svg width={W} height={H} style={{ position: 'absolute', inset: 0 }}>
        ...
        {COL_Y.map((y, i) => (
          <Flow key={'r' + i} accent={accent} prog={prog} p={stag(r.reason, i, 5)}
            d={`M 1000 ${ORB.y} C 1100 ${ORB.y} 1090 ${y} 1169 ${y}`} width={1.1} />
        ))}
        {COL_Y.map((y, i) => (
          <Flow key={'b' + i} accent={accent} prog={prog} p={stag(r.brain, i, 5)}
            d={`M 1381 ${y} C 1420 ${y} 1420 ${BRAIN.y} 1448 ${BRAIN.y}`} width={1.1} speed={4} />
        ))}
        {COL_Y.map((y, i) => (
          <Flow key={'a' + i} accent={accent} prog={prog} p={stag(r.actions, i, 5)}
            d={`M 1568 ${BRAIN.y} C 1600 ${BRAIN.y} 1600 ${y} 1634 ${y}`} width={1.1} speed={4} />
        ))}
        {/* bottom step arrows */}
        {[0, 1, 2].map((i) => {
          const p = clamp((r.words - 0.18 - i * 0.16) / 0.4, 0, 1);
          const mid = (STEP_X[i] + STEP_W[i] + STEP_X[i + 1]) / 2;
          const x0 = mid - 46, x1 = mid + 46;
          return (
            <path key={'s' + i} d={`M ${x0} 812 L ${x1} 812`} stroke="rgba(150,180,225,0.7)"
              strokeWidth="2" strokeLinecap="round" fill="none"
              pathLength="1" strokeDasharray="0.045 0.055" strokeDashoffset={0}
              opacity={p} style={{ clipPath: `inset(0 ${(1 - ec(p)) * 100}% 0 0)` }} />
          );
        })}
        <path d={`M 1700 852 C 1700 940 1300 946 900 946 C 500 946 250 936 250 856`}
          ...
          pathLength="1" strokeDasharray="1 1" strokeDashoffset={1 - ec(clamp((r.words - 0.6) / 0.4, 0, 1))} />
      </svg>
      ...
      {KEYWORDS.map(([w, x, y], i) => {
        ...
        return (
          <div key={w} style={{
            position: 'absolute', left: x, top: y + dy, transform: 'translate(-50%,-50%)',
            color: `rgba(190,210,245,${0.72 * p})`, font: `300 25px ${FONT}`, whiteSpace: 'nowrap',
          }}>{w}</div>
        );
      })}

      <Orb p={r.core} prog={prog} accent={accent} />

      {REASON.map((l, i) => (
        <Pill key={l} label={l} x={RX} y={COL_Y[i]} accent={accent}
          p={stag(r.reason, i, 5)} glow={i === 2} />
      ))}
      <BrainHub p={r.brain} prog={prog} accent={accent} />
      {ACTION.map((l, i) => (
        <Pill key={l} label={l} x={AX} y={COL_Y[i]} accent={accent}
          p={stag(r.actions, i, 5)} filled={i === 2} />
      ))}
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={s} style={{
            position: 'absolute', left: STEP_X[i], top: 812, transform: `translate(0,-50%) translateY(${(1 - ec(p)) * 14}px)`,
...
function Caption({ i, p }) {
  const o = seg(p, 0.06, 0.2) * (1 - seg(p, 0.86, 0.97));
  const [t, s] = CAPS[i] || CAPS[0];
  const vis = i === 4 ? 0 : o;
  return (
    <React.Fragment>
    <div style={{
      position: 'absolute', left: 0, right: 0, bottom: 0, height: 300, pointerEvents: 'none',
      opacity: vis, background: 'linear-gradient(to top, rgba(4,6,14,0.94) 12%, rgba(4,6,14,0.72) 45%, rgba(4,6,14,0) 100%)',
    }} />
    <div style={{
      position: 'absolute', left: 92, bottom: 82, opacity: vis,
      transform: `translateY(${(1 - ec(seg(p, 0.06, 0.2))) * 16}px)`,
    }}>
...
function makeScene(idx) {
  return function SceneBody({ accent, glow }) {
    const { progress } = useScene();
    const a = K[idx], b = K[idx + 1];
    const e = eio(progress);
    ...
    return (
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: BG }}>
        <div style={{
          ...
          <Diagram r={r} prog={progress} accent={accent} glow={glow} />
        </div>
        <Caption i={idx} p={progress} />
      </div>
    );
  };
}
...
const SCENE_MAP = {
  Observe: makeScene(0), Understand: makeScene(1), Decide: makeScene(2),
  Act: makeScene(3), Repeat: makeScene(4),
};

function FlowVideo() {
  const [t, setTweak] = useTweaks(window.TWEAK_DEFAULTS);
  const accent = t.accent || '#2f7dff';
  const wrap = (C) => (props) => <C {...props} accent={accent} glow={t.vividness} />;
  const map = {};
  Object.keys(SCENE_MAP).forEach((k) => { map[k] = wrap(SCENE_MAP[k]); });
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <SceneStage width={W} height={H} scenes={window.OM_SCENES} playback={window.OM_PLAYBACK} bg={BG}>
        {map}
      </SceneStage>
      <TweaksPanel>
        <TweakSection label="Look" />
        <TweakColor label="Accent" value={accent} options={['#2f7dff', '#22d3ee', '#7c5cff', '#31d68b']}
          onChange={(v) => setTweak('accent', v)} />
        <TweakSlider label="Vividness" value={t.vividness} min={0.6} max={1.8} step={0.05}
          onChange={(v) => setTweak('vividness', v)} />
